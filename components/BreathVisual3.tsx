...
// ── 빛줄기 정의 (60개) ─────────────────────────────────────────────
// t = (i+0.25)/59 → BV1(0), BV2(0.5) 사이 1/4 지점 채움
// maxLen / alpha / phase: BV2와 다른 해시 상수로 다양한 패턴
const STREAKS3 = Array.from({ length: 60 }, (_, i) => {
  const t      = (i + 0.25) / 59
  const angle  = -Math.PI * (0.95 - t * 0.90)
  const maxLen = 0.12 + ((i * 61 + 31) % 43) / 100       // 0.12 ~ 0.55
  const alpha  = 0.25 + ((i * 43 + 17) % 38) / 100       // 0.25 ~ 0.63
  const phase  = ((i * 113) % 40) / 100
  return { angle, phase, colorIdx: (i + 1) % 4, maxLen, alpha, dashes: randDashes(40) }
})

export default function BreathVisual3() {
